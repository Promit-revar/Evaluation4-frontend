...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    try {
      if (login) {
        const result = await makeRequest(loginUser, BASE_URL, {
          data: { email, password },
        });
        ...
        const result = await makeRequest(createUser, BASE_URL, {
          data: { email, password },
        });
        console.log(result);
        if (result.success) {
  ...
  return (
    <div className="LoginPage">
      <div className="login-card">
        <h1>
          Design API's Fast,<br></br>Manage Content Easily
        </h1>
        <img
      ...
      <div className="login-form">
        ...
        <div className="form">
          ...
          <a style={{ cursor: 'pointer' }} onClick={() => setLogin(!login)}>
            <u>{login ? "Don't have an account?" : 'go to login'}</u>
          </a>
        </div>
      </div>
    </div>
